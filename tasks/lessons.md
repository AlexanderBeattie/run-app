...
## Angular + Jest
- To test Angular `@Output() EventEmitter` emissions, use `jest.spyOn(component.output, 'emit')` and assert `toHaveBeenCalled()`. Do NOT subscribe to the EventEmitter and collect into an array — subscriptions don't reliably fire in Jest's synchronous TestBed environment without zone.js testing.
...
- Do not use any `any` types where possible
...
## Pace labels (standardised)
- `clubs.pace` and `run_events.pace` both use the same 5 labels: `easy` | `social` | `moderate` | `fast` | `tempo`
- Values are stored **lowercase** in the DB; UI capitalises via `text-transform: capitalize`
- `run-card.component.ts` `bannerGradient` maps these labels to CSS gradients — do not use time strings or custom values
- `schema.sql` enforces this with a CHECK constraint on `clubs.pace`; `run_events.pace` has no constraint (nullable, free-form, but convention is the 5 labels)
- Never use time strings (e.g. `5:30`, `5:00`) in seed data or forms — these break gradient mapping and are meaningless to users
