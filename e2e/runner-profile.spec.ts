...
test.describe('Runner profile card', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/api/runs/run-e2e-1/attendees', async route => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify([
          { id: 'user-e2e-1', display_name: 'Alice', has_verified_pace: false },
        ]),
      });
    });
    ...
    // Register AFTER loginTestUser so this specific mock wins LIFO over the generic **/api/users/** mock
  ...
  test('open run detail → click attendee → profile opens → Escape closes → run detail still visible', async ({ page }) => {
    ...
    await expect(page.locator('app-run-detail-dialog .dialog')).toBeVisible();

    // Click the attendee chip
    await page.getByRole('button', { name: /view profile for alice/i }).click();
    await expect(page.locator('app-runner-profile-dialog .card')).toBeVisible();
    await expect(page.locator('.name')).toContainText('Alice');

    // Press Escape to close the profile dialog
    await page.keyboard.press('Escape');
    await expect(page.locator('app-runner-profile-dialog .card')).not.toBeVisible();
  ...
  test('profile dialog close button dismisses without closing run detail', async ({ page }) => {
    ...
    await page.locator('app-runner-profile-dialog .card .close').click();
    await expect(page.locator('app-runner-profile-dialog .card')).not.toBeVisible();
    await expect(page.locator('app-run-detail-dialog .dialog')).toBeVisible();
  });
});
