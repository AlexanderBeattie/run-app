...
@Component({
  ...
  template: `
    <div class="page">
      ...
      <div class="form">
        <div class="field"><label>Run title</label><input [(ngModel)]="title" placeholder="e.g. Sunday morning 10k" /></div>
        <div class="field"><label>Start address</label><input [(ngModel)]="startAddress" placeholder="e.g. Victoria Park, London" /></div>
        <div class="field"><label>End address</label><input [(ngModel)]="endAddress" placeholder="e.g. Canary Wharf, London" /></div>
        <div class="field-row">
        ...
        <div class="field-row">
          <div class="field"><label>Distance (km)</label><input type="number" [(ngModel)]="distanceKm" placeholder="8" /></div>
          <div class="field"><label>Est. mins</label><input type="number" [(ngModel)]="estimatedMinutes" placeholder="50" /></div>
        </div>
        <div class="field"><label>Max attendees</label><input type="number" [(ngModel)]="maxAttendees" placeholder="Optional" /></div>
        <div class="field"><label>Notes</label><textarea [(ngModel)]="notes" rows="3" placeholder="Pace, meeting point..."></textarea></div>
        @if (error) { <div class="error">{{ error }}</div> }
        @if (loading) { <div class="info">Finding addresses...</div> }
        <button class="submit" [disabled]="loading" (click)="submit()">{{ loading ? 'Posting...' : 'Post run' }}</button>
      </div>
    </div>
  `,
  styles: [`
...
export class CreateRunComponent {
  ...
  title=''; startAddress=''; endAddress=''; date=''; time='';
  distanceKm=0; estimatedMinutes=0; maxAttendees?: number; notes='';
  error=''; loading=false;

  async submit() {
    if (!this.title || !this.startAddress || !this.endAddress || !this.date || !this.time) { this.error = 'Please fill in all required fields.'; return; }
    this.loading = true; this.error = '';
    const [sl, el] = await Promise.all([this.geo.geocode(this.startAddress), this.geo.geocode(this.endAddress)]);
    if (!sl) { this.error = 'Could not find start address.'; this.loading = false; return; }
    if (!el) { this.error = 'Could not find end address.'; this.loading = false; return; }
    const user = this.auth.getUser()();
    this.runsService.createRun({
      clubId: user?.clubId ?? null, clubName: user?.displayName ?? 'My Club',
      ...
      error: () => { this.error = 'Failed to post run.'; this.loading = false; }
    });
  }
}