...
@Component({
    ...
    <div class="page">
      ...
      <div class="form">
        @if (invalidToken()) {
          ...
          <div class="field"><label>New password</label><input type="password" [ngModel]="newPassword()" (ngModelChange)="newPassword.set($event)" placeholder="••••••••" autocomplete="new-password" /></div>
          <div class="field"><label>Confirm password</label><input type="password" [ngModel]="confirmPassword()" (ngModelChange)="confirmPassword.set($event)" placeholder="••••••••" autocomplete="new-password" (keyup.enter)="submit()" /></div>
  ...
  styles: [`
    ...
    .submit:disabled { background: #0F6E56; opacity: 0.7; cursor: not-allowed; }
